...
async function onCardHandler(reader, card){
    // MIFARE Classic is ISO/IEC 14443-3 tag
		...
		try {
            ...
            data.sectors[0] = {
                blocks: new Array(4),
                keyA: [255,255,255,255,255,255],
                keyB: [255,255,255,255,255,255],
                accessBytes: [255,15,0],
            };
            for(var b = 0; b < 4; b++){
                var blockData = await reader.read(0, 16, 16);
                data.sectors[0].blocks[b] = [...blockData];
            }

            pretty.info("gen card", data);
            ...
            for(var sector = 1; sector < 16; sector++){
                ...
                for(var b = 0; b < 4; b++){
                    ...
                    await reader.write(block, Buffer.from(data.sectors[sector].blocks[b]), 16);
                }
